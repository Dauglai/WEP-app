...
export class AccountService {
  ...
  createStudent(Token: any, hero: number, hero_name: string): Observable<any> {
    const myHeaders = new HttpHeaders(
      {'Content-Type': 'application/json', Authorization: 'Token ' + Token}
    );
    const student = { hero: hero, hero_name: hero_name };
    return this.http.post(this.apiCreateStudent, student, {headers: myHeaders});
  }
